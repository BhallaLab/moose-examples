...
class IonChannel(moose.HHChannel):
    ...
    def beta_m(self):
        if self.Xpower == 0:
            return numpy.array([])
        return numpy.array(
            moose.element("%s/gateX" % (self.path)).tableB
    ...
    def beta_h(self):
        if self.Ypower == 0:
            return numpy.array([])
        return numpy.array(
            moose.element("%s/gateY" % (self.path)).tableB
...
class SquidAxon(moose.Compartment):
    ...
    def reversal_potential(cls, temp, c_out, c_in):
        ...
        v = (
            (GAS_CONSTANT * temp / FARADAY) * 1000.0 * numpy.log(c_out / c_in)
            + 70.0
            + cls.EREST_ACT
        )
        return v
...
class SquidModel(moose.Neutral):
    ...
    def __init__(self, path):
        ...
        print(
            (
                "Current clamp connected:",
                moose.connect(
                    self.current_clamp, "output", self.squid_axon, "injectMsg"
                ),
            )
        )
